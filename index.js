...
//   try {
//     // Load your GeoJSON (either via fetch or direct import)
//     const response = await fetch('./mapData.geojson');
//     const geoJson = await response.json();
//     console.log('GeoJSON data:', geoJson);
//     // Register the GeoJSON with ECharts
//     echarts.registerMap('washington', geoJson);
//     console.log('Available maps:', echarts.getMap('washington'));
//     // Initialize your chart
//     const mapInstance = echarts.init(document.getElementById('map'))
//     //map bounds
//     const bounds = {
//       left: -124.7628,
//       right: -116.9161,
...
//     // Initialize with explicit renderer
//     const mapInstance = echarts.init(mapContainer, null, {
//       renderer: 'canvas' // Explicitly use canvas renderer for better compatibility
//     });

//     // Show loading state
//     mapInstance.showLoading();

//     // Load GeoJSON with explicit error handling
//     let geoJson;
//     try {
//       const response = await fetch('./mapData.geojson');
//       if (!response.ok) {
...
//       mapContainer.innerHTML = 'Failed to load map data';
//       return;
//     }

//     // Register map with error handling
//     try {
//       echarts.registerMap('washington', geoJson);
//     } catch (error) {
//       console.error('Failed to register map:', error);
//       mapContainer.innerHTML = 'Failed to initialize map';
//       return;
//     }
...
//       },
//       tooltip: {
//         trigger: 'item', // Changed from 'position: top'
//         formatter: (params) => {
//           if (params.seriesName === 'pins') {
//             return params.name;
//           }
...
//           fontWeight: 'bold',
//         },
//         roam: false, // Disable zooming/panning for better compatibility
//         zoom: 1,
//         itemStyle: {
//           borderColor: '#fff',
//           borderWidth: 2,
...
//           data: establishedSites,
//           symbol: 'pin',
//           symbolSize: 20, // Reduced size for better rendering
//           itemStyle: {
//             color: '#000',
//             borderColor: '#fff',
//             borderWidth: 2
...
//     } catch (error) {
//       console.error('Failed to set map options:', error);
//       mapContainer.innerHTML = 'Failed to configure map';
//       return;
//     }
...
//     const mapContainer = document.getElementById('map');
//     if (mapContainer) {
//       mapContainer.innerHTML = 'Failed to initialize map. Please check console for details.';
//     }
//   }
// }
...
async function createMapChart() {
  try {
    ...
    try {
      const response = await fetch('./mapData.geojson');
      geoJson = await response.json();
      console.log('GeoJSON loaded successfully:', geoJson.type);
    } catch (error) {
      console.error('Failed to load GeoJSON:', error);
      return;
    }
    ...
    const establishedSites = [
      {name: 'Behavioral Health Resources', value: [-122.8405, 47.0458], counties: 'Thurston and Mason'},
      {name: 'Behavioral Health Resources', value: [-123.8829, 46.95], counties: 'Grays Harbor'},
      {name: 'Cascade Community Healthcare', value: [-122.9832, 46.7349], counties: 'Lewis and Cowlitz'},
      {name: 'Catholic Charities Serving Central Washington', value: [-120.3056, 47.4234], counties: 'Douglas and Chelan'},
      {name: 'Comprehensive Healthcare', value: [-120.5124, 46.5948], counties: 'Yakima'},
      {name: 'Comprehensive Healthcare', value: [-119.1142, 46.2528], counties: 'Franklin'}, 
      {name: 'Lummi Nation Tribal Health Center', value: [-122.6215, 48.7944], counties: 'Whatcom'},
      {name: 'Frontier Behavioral Health', value: [-117.2834, 47.6676], counties: 'Spokane'}
    ];
    ...
    const option = {
      ...
      tooltip: {
        trigger: 'item', // Shows tooltip for both map areas and scatter points
        formatter: function(params) {
          // Different tooltip content based on series type
          if (params.seriesName === 'pins') {
            return `<strong>${params.name}</strong><br/>
                    Counties Served: ${params.data.counties}`;
          } else {
            // For map regions
            return `<strong>${params.name} County</strong><br/>
                    Coverage: ${params.value === 1 ? 'Yes' : 'No'}`;
          }
        },
      ...
      series: [
        ...
        {
          name: 'pins',
          type: 'scatter',  // Changed to effectScatter for better compatibility
          coordinateSystem: 'geo',
          data: establishedSites,
